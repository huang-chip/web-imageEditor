...
export default {
  ...
  methods: {
    ...
    handlePositionChange() {
      ...
      const cropper = this.$refs.cropper.cropper;
      const canvasData = cropper.getCanvasData(); // 获取图片画布数据

      // 将输入框的值直接设置为裁剪框的相对位置
      const x = this.cropperLeft; // 直接使用输入框的值
      const y = this.cropperTop; // 直接使用输入框的值

      // 确保不超出图片边界
      const boundX = Math.max(0, Math.min(x, canvasData.width - this.width));
      const boundY = Math.max(0, Math.min(y, canvasData.height - this.height));
    ...
    handleCropChange(data) {
      if (!this.isCropping) {
        ...
        this.cropperLeft = Math.round(data.detail.x); // 直接使用裁剪框的 x 坐标
        this.cropperTop = Math.round(data.detail.y); // 直接使用裁剪框的 y 坐标
      }
    },
  },
};
